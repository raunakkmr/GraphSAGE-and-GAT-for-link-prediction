...
class TemporalNetworkSNAP(Dataset):

    def __init__(self, path, neg_examples_path='', mode='train',
        ...
        neg_examples_path : str
            Path to file with negative examples. If empty string, then negative examples are generated and saved in the same directory as the dataset file as ${dataset file name}_neg_examples_${mode}.txt. Default: ''.
        mode : str
        ...
        self.path = path
        self.neg_examples_path = neg_examples_path
        self.mode = mode
        ...
        if neg_examples_path:
            ...
            save_path = os.path.splitext(path)[0] + '_neg_examples_{}'.format(mode)
...
class CollegeMsg(TemporalNetworkSNAP):
...
class BitcoinAlpha(TemporalNetworkSNAP):
    ...
    def _read_from_file(self, path):
        ...
        edges_all = np.concatenate((edges_all[:, :2], edges_all[:, 3:]), axis=1)

        return edges_all

class CollegeMsgOld(Dataset):
    ...
    def __init__(self, path, neg_examples_path='', mode='train',
        ...
        neg_examples_path : str
            Path to file with negative examples. If empty string, then negative examples are generated and saved in the same directory as CollegeMsg.txt as CollegeMsg_neg_examples_${mode}.txt. Default: ''.
        mode : str
            One of train, val or test. Default: train.
        num_layers : int
            Number of layers in the computation graph. Default: 2.
        ...
        edges_all = np.loadtxt(path, dtype=np.int64)
        print('Finished reading data.')
        ...
        pos_examples = pos_examples[:, :2]
        pos_examples = np.array([row for row in pos_examples \
                                 if ((row[0], row[1]) not in seen) and
                                 (row[0] < self.n) and
                                 (row[1] < self.n)])
        if neg_examples_path:
            neg_examples = np.loadtxt(neg_examples_path)
            ...
            cur = 0
            n, _choice = self.n, np.random.choice
            seen = set(tuple(e[:2]) for e in edges_all)
            while cur < num_neg_examples:
                u, v = _choice(n, 2, replace=False)
                if (u, v) in seen:
                    continue
        ...
        save_path = os.path.join(os.path.dirname(path),
                                 'CollegeMsg_neg_examples_{}.txt'.format(mode))
        np.savetxt(save_path, neg_examples)

        # pos_examples = pos_examples[:1024]
        # neg_examples = neg_examples[:1024]
