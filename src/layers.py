...
# class GraphAttention(nn.Module):
# 
#     def __init__(self, dim, num_heads, dropout=0.5):
#         """
#         Parameters
#         ----------
#         dim : int
...
#         self.num_heads = num_heads
# 
#         self.fc = nn.Linear(2*dim, num_heads, bias=False)
#         self.dropout = nn.Dropout(dropout)
#         self.leakyrelu = nn.LeakyReLU()
# 
#     def forward(self, features, nodes, mapping, rows):
...
#         indices = torch.LongTensor([[mapping[nodes[i]], c] for i in range(len(rows)) for c in rows[i]]).t()
# 
#         nbr_h = torch.cat(tuple([features[row] for row in rows if len(row) > 0]), dim=0)
#         self_h = torch.cat(tuple([features[mapping[nodes[i]]].repeat(len(row), 1) for (i, row) in enumerate(rows) if len(row) > 0]), dim=0)
#         cat_h = torch.cat((self_h, nbr_h), dim=1)
# 
#         e_arr = self.leakyrelu(self.fc(cat_h))
# 
#         alpha_arr = [F.softmax(e_arr[sum_degs[i] : sum_degs[i+1]], dim=0) for i in range(len(sum_degs)-1)]
#         alpha_arr = torch.cat(tuple(alpha_arr), dim=0)
#         alpha_arr = self.dropout(alpha_arr)
# 
#         # alpha_arr = torch.ones(alpha_arr.shape)
# 
#         adj_arr = [torch.sparse.FloatTensor(indices, alpha_arr[:, head], torch.Size([nprime, nprime])) for head in range(self.num_heads)]
#         out = [torch.sparse.mm(adj, features)[mapped_nodes] for adj in adj_arr]
# 
#         return out 